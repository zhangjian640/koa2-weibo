...
const { Z_ID, Z_USER_NAME, Z_COOKIE, L_ID, L_USER_Name } = require('../testUserInfo')

describe('关注和取消关注', () => {
  // 先让张三取消关注李四（为了避免张三已经关注了李四）
  test('无论如何，先取消关注，应该成功', async () => {
    const res = await server
      .post('/api/profile/unFollow')
      .send({userId: L_ID})
      .send('cookie', Z_COOKIE)
    expect(1).toBe(1)
  })
  ...
  test('获取李四的粉丝，应该有张三', async () => {
    ...
    const hasUserName = fansList.some(fansInfo => {
      return fansInfo.userName === Z_USER_NAME
    })
